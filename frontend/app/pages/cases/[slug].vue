...
useSeoMeta({
	title: () => project.value?.seoGroup?.metaTitle || `${project.value?.title} | Portfolio`,
	description: () => project.value?.seoGroup?.metaDescription || project.value?.task,
	ogTitle: () => project.value?.seoGroup?.metaTitle || project.value?.title,
	ogDescription: () => project.value?.seoGroup?.metaDescription || project.value?.task,
...
<template>
	<article class="container mx-auto px-4 py-20 max-w-5xl">
		<div v-if="pending" class="py-32 text-center text-muted-foreground animate-pulse">
			Loading case...
		</div>
		...
		<div v-else-if="project">
			<header class="mb-16 text-center max-w-3xl mx-auto space-y-8">
				...
				>
					<div class="flex flex-col items-center">
						<span class="text-xs uppercase tracking-widest font-semibold mb-1">Client</span>
						<span class="text-foreground font-medium">{{ project.client }}</span>
					</div>
					<div v-if="project.service" class="flex flex-col items-center">
						<span class="text-xs uppercase tracking-widest font-semibold mb-1">Service: </span>
						<NuxtLink
					...
					<div class="flex flex-col items-center">
						<span class="text-xs uppercase tracking-widest font-semibold mb-1">Task</span>
			...
			>
				...
				/>
				<div v-else class="w-full h-full bg-muted flex items-center justify-center">No cover</div>
			</div>

			<section v-if="project.gallery?.length" class="mb-16 space-y-6">
				<h2 class="text-2xl font-bold">Gallery</h2>
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div v-for="(item, idx) in project.gallery" :key="idx" class="group space-y-2">
						<div class="rounded-xl overflow-hidden border bg-muted aspect-video">
							<img
								:src="item.url || ''"
						...
						<p v-if="item.caption" class="text-xs text-muted-foreground text-center">
							{{ item.caption }}
						</p>
					</div>
				</div>
			...
			<section
				v-if="project.result"
				v-gsap.entrance.slide-bottom="{delay: 0.3}"
		...
		<div v-else class="py-32 text-center">
			...
			<NuxtLink to="/cases" class="text-primary hover:underline">Return to portfolio</NuxtLink>
		</div>
	</article>
</template>
