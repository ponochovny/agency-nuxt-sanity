	...
	<div class="py-20 container mx-auto px-4 max-w-6xl">
		<div class="text-center max-w-3xl mx-auto mb-16 space-y-4">
			<span
				...
				class="text-xs uppercase font-bold tracking-widest text-primary"
				>How we help</span
			>
			<h1
				v-gsap.entrance.slide-bottom.delay-300
				class="text-4xl sm:text-6xl font-extrabold tracking-tight"
			>
				Our Services
			</h1>
			<p v-gsap.entrance.slide-bottom.delay-500 class="text-xl text-muted-foreground">
				We transform complex business challenges into understandable digital products with high
				conversion rates.
			</p>
		</div>

		<div v-if="pending" class="text-center py-20 text-muted-foreground">Loading services...</div>
			...
			>
				...
				>
					<div class="flex items-center justify-between mb-4">
						<span class="text-xs font-semibold px-3 py-1 bg-primary/10 text-primary rounded-full">
							{{ service.badge || 'Service' }}
						</span>
						<span class="text-xs text-muted-foreground font-mono">{{ service.timeframe }}</span>
					</div>
		...
		>
			<h3 class="text-2xl sm:text-3xl font-bold">Need an unconventional solution?</h3>
			<p class="text-muted-foreground max-w-xl mx-auto">
				Describe your task, and we will select a customized technology stack and team for your
				project.
			</p>
			<NuxtLink
				to="/contact"
